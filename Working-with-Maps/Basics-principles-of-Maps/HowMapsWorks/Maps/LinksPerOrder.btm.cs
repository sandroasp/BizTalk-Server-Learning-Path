namespace HowMapsWorks.Maps {
    ...
    public sealed class LinksPerOrder : global::Microsoft.XLANGs.BaseTypes.TransformBase {
  ...
  <xsl:template match=""/s0:PeopleOrigin"">
    <ns0:PeopleTarget>
      ...
      <xsl:for-each select=""Employee"">
        <xsl:variable name=""var:v2"" select=""string(Sex/text())"" />
        <xsl:variable name=""var:v3"" select=""userCSharp:LogicalEq($var:v2 , &quot;F&quot;)"" />
        <xsl:if test=""$var:v3"">
          <Person>
            <Name>
              <xsl:value-of select=""Name/text()"" />
